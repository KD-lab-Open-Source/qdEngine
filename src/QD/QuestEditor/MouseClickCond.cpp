#include "stdafx.h"
#include "resource.h"
#include ".\mouseclickcond.h"

MouseClickCond::MouseClickCond(DWORD id, CWnd* parent /*= NULL*/):
OneObjectCond(id, parent)
{
	objectComboStaticText_.LoadString(IDS_MOUSE_CLICK_ON);
}

MouseClickCond::~MouseClickCond(void)
{
}

void MouseClickCond::LoadObjects()
{
	__super::LoadObjects();

	m_wndObj.InsertString(0, CString((LPTSTR)IDS_COND_MOUSE_CLICK_OBJ_SELF));
	m_wndObj.SetCurSel(0);
	OnSelchangeComboObject();
}

void MouseClickCond::OnSelchangeComboObject() 
{
	int iSel = m_wndObj.GetCurSel();
	if(iSel == -1)
		return;
	DWORD dwData = m_wndObj.GetItemData(iSel);

	qdCondition::ConditionDataIndex eIdx = qdCondition::OBJECT_NAME;
	if(dwData)
	{
		qdNamedObject* const p = 
			reinterpret_cast<qdNamedObject*>(m_wndObj.GetItemDataPtr(iSel));
		m_qdCondition.put_object(eIdx, p);
	}
	else
		m_qdCondition.put_object(eIdx,NULL);

	m_ptrMakeCondDlg->OnCondDataComplete(IsCondDataComplete());
}
