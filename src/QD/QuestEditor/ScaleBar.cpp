...
{
	bool bEnable = qdGameDispatcher::get_dispatcher() != NULL;
	pCmdUI->Enable(bEnable);
	m_wndCombo.EnableWindow(bEnable);
}
...
BOOL CScaleBar::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN&&pMsg->wParam == VK_RETURN)
	{
		...
		m_wndCombo.GetComboBoxInfo(&cbi);
		if (pMsg->hwnd == cbi.hwndItem)
		{
			ReadAndSetScale();
			return TRUE;
		}
