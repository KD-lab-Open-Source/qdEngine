...
void CShellTree::PopulateTree() 
{
	...
	HTREEITEM hItem = GetMyComputerNode();
...
/*
 *	Наша цель найти узел "Moй компьютер"
 */
HTREEITEM CShellTree::GetMyComputerNode()
{
	...
	if (!lptvid || !lptvid->lpsfParent)
		return NULL; 

	STRRET strDispName;
	TCHAR strMyCompDispName[MAX_PATH];

	lptvid->lpsfParent->GetDisplayNameOf(pidlRoot, SHGDN_NORMAL, &strDispName);
	StrRetToBuf(&strDispName, pidlRoot, strMyCompDispName,sizeof(strMyCompDispName));
	pMalloc->Free(pidlRoot);

	CString strText;
	do {
		strText = GetItemText(hItem);
		if (strText.CompareNoCase(strMyCompDispName) == 0)
			return hItem;
...
{
	HTREEITEM subNode = GetMyComputerNode();
	CString szPathHop;
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
	char fname[_MAX_FNAME];
	...
	if(subNode)
	{
		if(SearchTree(subNode,szPathHop, CShellTree::type_drive))
		{
			...
			while(p)
			{
				...
				{
					TRACE("Search folder failed: szPathHop = %s, subNode text = %s",
						szPathHop, static_cast<LPCTSTR>(GetItemText(subNode)));
					p=NULL;
				}
			}
		}
		...
		{
			TRACE("Search drive failed: szPathHop = %s, subNode text = %s",
				szPathHop, static_cast<LPCTSTR>(GetItemText(subNode)));
		}
	}
	else
		TRACE("GetChildItem returned NULL");
}

/////////////////////////////////////////////////////////////////////
// notify messages reflection
...
void CShellTree::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
