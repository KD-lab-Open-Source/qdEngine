...
BOOL InitLayeredWindows()
{
	if( g_lpfnUpdateLayeredWindow == NULL || g_lpfnSetLayeredWindowAttributes == NULL )
	{
		...
		g_lpfnUpdateLayeredWindow =	
			(lpfnUpdateLayeredWindow)GetProcAddress( hUser32, _T("UpdateLayeredWindow") );
		...
		if( g_lpfnUpdateLayeredWindow == NULL 
			|| g_lpfnSetLayeredWindowAttributes == NULL )
			return FALSE;
	}
...
BOOL CTreeListTipCtrl::Show( CRect rcItem, LPCTSTR lpszText, UINT uFormat )
{
	...
	if( m_rcItem != rcItem )// || !IsWindowVisible() )
	{
		...
		if( size.cx + 5 > rcItem.Width() )
		{
			...
			m_rcTip		= rcItem;
			m_rcTip.OffsetRect(2, GetSystemMetrics(SM_CYCURSOR));
