	...
	m_pDropWnd( NULL )
{
	...
	// load cursors
	m_hSize		= treelist_res::GetHeaderSizeCursor();
	m_hSplit	= treelist_res::GetHeaderSplitCursor();
...
HCURSOR CTreeListHeaderCtrl::GetSplitCursor() const{
	return m_hSplit;
}
void	CTreeListHeaderCtrl::SetSplitCursor(HCURSOR cursor){
	if (cursor)
		m_hSplit = cursor;
	else
		m_hSplit = GetDefaultCursor();
}

HCURSOR CTreeListHeaderCtrl::GetResizeCursor() const{
	return m_hSize;
}
void CTreeListHeaderCtrl::SetResizeCursor(HCURSOR cursor){
	if (cursor)
		m_hSize = cursor;
	else
		m_hSize = GetDefaultCursor();
}

HCURSOR CTreeListHeaderCtrl::GetDefaultCursor() const{
	return m_hArrow;
}
void CTreeListHeaderCtrl::SetDefaultCursor(HCURSOR cursor){
	ASSERT(cursor);
	m_hArrow = cursor;
}
...
{
	if( !m_imgSort.Create(8, 16, ILC_MASK|ILC_COLOR24, 5, 3))
		return FALSE;

//	// create sort image
	if( m_imgSort.Add( treelist_res::GetDefsortBitmap(), 0xFF00FF ) == -1)
		return FALSE;

	if( !m_imgHeader.Create( 16, 16, ILC_MASK|ILC_COLOR24, 0, 4) )
		return FALSE;

	if( m_imgHeader.Add( treelist_res::GetDefheadBitmap(), 0xFF00FF ) == -1)
		return FALSE;
...
BOOL CTreeListHeaderCtrl::Create( DWORD dwStyle, 
								 RECT const& rect, 
								 CTreeListCtrl* pParentWnd, 
								 UINT nID )
{
	// create window
	...
	if( !CWnd::Create( TREELISTHEADERCTRL_CLASSNAME, 
